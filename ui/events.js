...
function handleDocumentClick(e) {
    ...
    // ============================================
    // FIX: Skip FORM elements ONLY when the click target
    // is an input/select/textarea inside the form, NOT when
    // clicking on buttons that have data-action
    // ============================================
    // Check if we clicked on a form element itself (not a button with data-action)
    ...
    // Only skip FORM if:
    // 1. The action element is a FORM (not a button inside it)
    // 2. AND we didn't click on a button or actionable element
    if (isFormElement && !isButtonClick) {
        // This prevents form validation on input field clicks
        return;
    }
